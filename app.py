...
st.markdown("""
    ...
    .can {
        width: 40px; height: 90px; border-radius: 6px; position: relative;
        border: 2px solid #222; box-shadow: 5px 5px 15px rgba(0,0,0,0.8);
    }
...
if messaggi:
    ...
    <style>
        .wall-canvas {
            display: flex;
            flex-wrap: wrap;
            justify-content: space-around; /* Sparparagliamento orizzontale */
            align-items: center;
            padding: 60px 20px;
            width: 100%;
        }
        .graffito-item {
            ...
            align-items: center;
            /* Margini randomici per rompere la griglia */
        ...
        .text-neon {
            ...
            z-index: 2;
            text-shadow: 1px 1px 2px #000, 0 0 12px var(--c);
        }
        .drip {
    ...
    for m in messaggi:
        random.seed(m['id'])
        # Generiamo spostamenti randomici per ogni pezzo
        ...
        '''
    st.components.v1.html(f"{style_block}<div class='wall-canvas'>{content_html}</div>", height=3000, scrolling=False)
